    ...
    def update_clicks(self):
        if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT): # seulement clic gauche
            ...
            if self.hover == "menu_open":
                ...
                pass # TODO implémenter le choix de la pièce lors de la promotion
            elif self.hover == "promote_rook":
                pass # TODO implémenter le choix de la pièce lors de la promotion
            elif self.hover == "promote_bishop":
                pass # TODO implémenter le choix de la pièce lors de la promotion
    ...
    def en_passant(self):
        ...
        if self.player_turn == 0:  # prise en passant pour les blancs
            for piece in self.pieces_white:
                if piece.type == "pawn" and piece.y == 4:  # pion blanc sur la 5e rangée
                    for enemy_piece in self.pieces_black:  # vérifie si un pion noir vient de se déplacer de 2 cases
                        if enemy_piece.type == "pawn" and enemy_piece.y == 4 and abs(piece.x - enemy_piece.x) == 1: # pion noir adjacent
                            if enemy_piece.has_moved and abs(enemy_piece.y - 6) == 2:  # pion noir vient de avancer de 2
                                piece.possible_moves.append((enemy_piece.x, 5))  # ajouter la capture en passant aux mouvements possibles
        else:  # prise en passant pour les noirs
            for piece in self.pieces_black:
                if piece.type == "pawn" and piece.y == 3:  # pion noir sur la 4e rangée
                    for enemy_piece in self.pieces_white:  # vérifie si un pion blanc vient de se déplacer de 2 cases
                        if enemy_piece.type == "pawn" and enemy_piece.y == 3 and abs(piece.x - enemy_piece.x) == 1: # pion blanc adjacent
                            if enemy_piece.has_moved and abs(enemy_piece.y - 1) == 2:  # pion blanc vient de avancer de 2
                                piece.possible_moves.append((enemy_piece.x, 2)) # ajouter la capture en passant aux mouvements possibles
